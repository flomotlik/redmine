    ...
    class Gantt
      ...
      def line_for_project(project, options)
        ...
        if project.is_a?(Project) && project.start_date && project.due_date
          ...
          coords = coordinates(project.start_date.try(:to_date), project.due_date.try(:to_date), nil, options[:zoom])
          label = h(project)
          case options[:format]
          when :html
      ...
      def line_for_issue(issue, options)
        ...
        if issue.is_a?(Issue) && issue.due_before
          coords = coordinates(issue.start_date.try(:to_date), issue.due_before.try(:to_date), issue.done_ratio, options[:zoom])
