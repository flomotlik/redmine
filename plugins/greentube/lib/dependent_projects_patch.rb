module DependentProjectsPatch
  ...
  module InstanceMethods
    def statement_with_dependent_projects
      statement = statement_without_dependent_projects
      if project
        statement = "(#{statement}) OR (issues.id IN (SELECT issues.id FROM issues LEFT OUTER JOIN custom_values ON custom_values.customized_type='Issue' AND issues.id=customized_id
